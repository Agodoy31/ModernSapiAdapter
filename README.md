...
Providers are no longer built here. Any providers (Azure, Google, Piper, VITS) are now completely autonomous, standalone executables written in any language (C#, Python, Rust) that simply expose the IPC pipes. If a provider crashes, the pipe closes, the CoreEngine recovers gracefully, and the host application never skips a beat.
